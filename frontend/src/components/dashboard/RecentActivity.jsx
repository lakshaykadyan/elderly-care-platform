      ...
      {activities.length === 0 ? (
        ...
        <div className="recent-list">
          {activities.map((service) => (
            <div key={service._id} className="recent-card" title="View Recent Service">
              <div className="recent-left">
                <div className="recent-avatar">
                  {service.userId?.name?.charAt(0)?.toUpperCase() || <User size={20} />}
                </div>
                <div>
                  <h3>{service.userId?.name}</h3>
                  <p style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <Stethoscope size={14} /> {service.serviceType}
                  </p>
                </div>
              </div>
